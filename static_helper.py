...
def generate_static_info(cfg_dir, cpg_dir, format_dir, jelly_cg_path, joern_dir, pickle_path, overwrite, package_dir,
                         pdg_dir, entry_script_set):
    if os.path.exists(joern_dir) and os.path.exists(pdg_dir) and os.path.exists(
            cfg_dir) and os.path.exists(cpg_dir) and not overwrite and os.path.exists(pickle_path):
...
def install_dependencies(cwd: str, timeout: int = 120):
    ...
    command = ['npm', 'install', '--ignore-scripts', '--no-audit', '--production', '--registry',
