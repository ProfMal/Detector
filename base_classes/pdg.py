...
class PDG:
    def __init__(self, pdg_path, cpg, pdg_graph=None):
        ...
        self.line_number = int(pdg.nodes[first_node_id]['LINE_NUMBER']) if 'LINE_NUMBER' in pdg.nodes[
            first_node_id] else None

        # the end line number of the source code corresponding to the pdg
        self.line_number_end = int(pdg.nodes[first_node_id]['LINE_NUMBER_END']) if 'LINE_NUMBER_END' in pdg.nodes[
            first_node_id] else None

        # the start column number of the source code corresponding to the pdg
        self.column_number = int(pdg.nodes[first_node_id]['COLUMN_NUMBER']) if 'COLUMN_NUMBER' in pdg.nodes[
            first_node_id] else None

        # the end column number of the source code corresponding to the pdg
        self.column_number_end = int(pdg.nodes[first_node_id]['COLUMN_NUMBER_END']) if 'COLUMN_NUMBER_END' in pdg.nodes[
            first_node_id] else None
