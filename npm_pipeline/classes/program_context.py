...
class ProgramContext:
    def __init__(self, file_list: list[str]):
        nodejs_global_object_list = ['Object', 'process', 'global', 'Buffer', 'Blob', 'console', 'require', 'JSON',
                                     'document', 'Math', 'Date', 'Array', 'String', 'Number', 'Boolean', 'RegExp',
                                     'Symbol', 'BigInt', 'Promise', 'Set', 'Map', 'WeakSet', 'WeakMap', 'Intl', 'fetch',
                                     'Error', 'TypeError', 'ReferenceError', 'SyntaxError', 'RangeError', 'EvalError',
                                     'TextEncoder']
        nodejs_core_module_list = ['assert', 'buffer', 'child_process', 'cluster', 'crypto', 'dgram', 'dns', 'domain',
                                   'events', 'fs', 'fs/promises', 'http', 'https', 'net', 'os', 'path', 'punycode',
                                   'querystring', 'readline', 'stream', 'string_decoder', 'timers', 'tls', 'tty', 'url',
                                   'util', 'v8', 'vm', 'zlib']
        global_builtin_function_list = ['encodeURIComponent', 'decodeURIComponent', 'encodeURI', 'decodeURI', 'escape',
                                        'unescape', 'parseInt', 'parseFloat', 'isNaN', 'isFinite', 'eval', 'fetch',
                                        'Buffer', 'setTimeout', 'btoa']
        ...
        for name, item in self.global_object_dict.items():
            ...
            identifier = Identifier(name=name, line_number=None, column_number=None, node_id=None, file=None,
                                    source_pdg=None, identifier_type=GLOBAL_OBJECT, identifier_cat='BUILTIN')
        ...
        for name, item in self.builtin_function_object_dict.items():
            # it is the dummy identifier, in order to keep the find logic general
            identifier = Identifier(name=name, line_number=None, column_number=None, node_id=None, file=None,
                                    source_pdg=None, identifier_type=GLOBAL_OBJECT, identifier_cat='FUNCTION_REF')
            identifier.set_ref_object(item)
            self.global_identifier_dict[name] = identifier
