...
class APICall:
    def __init__(self, _type: str, timestamp: str, module: str, function: str,
                 caller: Dict[str, Any], arguments: List[Any], result: Any):
        """
...
class APICallCollection:
    ...
    def add_api_call(self, api_call: APICall):
        ...
        if (file_val is not None and start_line_val is not None and start_column_val is not None and
                end_line_val is not None and end_column_val is not None):
            # Remove any API call with matching caller file, line, and column.
            self.collections = [
                call for call in self.collections
                if not (
                        call.caller.get("file") == file_val and
                        call.caller.get("start_line") == start_line_val and
                        call.caller.get("start_column") == start_column_val and
                        call.caller.get("end_line") == end_line_val and
                        call.caller.get("end_column") == end_column_val
                )
            ]
        # Append the new API call at the end.
        self.collections.append(api_call)

    def find_api_call(self, _type: str, file: str, start_line: int, start_column: int, end_line: int,
                      end_column: int) -> Optional[Tuple[str, str]]:
        """
        ...
        for api_call in self.collections:
            ...
            if (_type == call_type and caller.get("file") == file
